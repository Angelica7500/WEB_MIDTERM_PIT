...
Line 46:10 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 47:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 57:14 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
Line 61:9 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
...
class PredictiveSearch extends HTMLElement {
  ...
  setupEventListeners() {
    ...
    this.querySelector('button[type="button"]').addEventListener('click', this.clear.bind(this));
  ...
  getSearchResults(searchTerm) {
    ...
    fetch(`${routes.predictive_search_url}?q=${encodeURIComponent(searchTerm)}&${encodeURIComponent('resources[type]')}=product&${encodeURIComponent('resources[limit]')}=${perPage}&section_id=predictive-search`)
      .then((response) => { 
        if (!response.ok) {
  ...
  clear() {
    ...
    if (!this.modal) this.input.focus();
  }
}
